...
@pytest.fixture
def relation_graph() -> "RelationGraphPort":
    """Create mock relation graph."""
    rg = AsyncMock()
    rg.count_entities = AsyncMock(return_value=150)
    rg.count_relations = AsyncMock(return_value=75)
    return rg
...
class TestRepositoryStatsUpdater:
    ...
    async def test_updates_all_stats_after_file_indexed(
        self, state_db: "IndexingStatePort", relation_graph: "RelationGraphPort"
    ) -> None:
        """Updater should query all counts and update repository stats."""
        updater = RepositoryStatsUpdater(state_db, relation_graph)
        repo_id = str(uuid4())

        await updater.update_after_file_indexed(repo_id)

        # Should query file/pending counts from state_db
        ...
        # Should query entity/relation counts from relation_graph
        relation_graph.count_entities.assert_awaited_once_with(repo_id)
        relation_graph.count_relations.assert_awaited_once_with(repo_id)
    ...
    async def test_includes_current_timestamp(
        self, state_db: "IndexingStatePort", relation_graph: "RelationGraphPort"
    ) -> None:
    ...
    async def test_updates_relation_count_only_after_resolution(
        ...
        relation_graph.count_relations = AsyncMock(return_value=100)
        updater = RepositoryStatsUpdater(state_db, relation_graph)
        repo_id = str(uuid4())

        await updater.update_after_resolution(repo_id)

        # Should only query relation count from relation_graph (not all counts)
        relation_graph.count_relations.assert_awaited_once_with(repo_id)
        state_db.count_indexed_files.assert_not_awaited()
        relation_graph.count_entities.assert_not_awaited()
